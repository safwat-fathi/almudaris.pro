...
export function ChildrenListClient({ initialChildren, teachers }: { initialChildren: Child[], teachers: Teacher[] }) {
  const [selectedChild, setSelectedChild] = useState<Child | null>(null);
  const [isBottomSheetOpen, setIsBottomSheetOpen] = useState(false);

  const handleOpenAssignTeacher = (child: Child) => {
    setSelectedChild(child);
    setIsBottomSheetOpen(true);
  };

  const handleAssignTeacher = async (childId: number, teacherId: number) => {
    // Backend call for actual enrollment would go here
    // POST /parents/children/:childId/enroll { teacher_id: teacherId }
    // For now we simulate success and perhaps refresh or show toast
    console.log("Assigned teacher", teacherId, "to child", childId);
  };
    ...
    <>
      <div className="flex-1 overflow-y-auto px-6 md:px-12 space-y-4 pb-28 pt-2 relative z-10 w-full max-w-2xl mx-auto">
        {initialChildren.map((child: Child) => (
          <div key={child.id} className="relative p-6 rounded-2xl bg-surface-container-lowest border border-outline-variant/10 shadow-[0_8px_24px_rgba(0,0,0,0.04)] pb-8 mb-4">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex-1">
                <h2 className="font-headline font-extrabold text-xl text-on-surface">{child.name}</h2>
                {child.email && (
                  <p className="font-body text-outline font-medium text-sm mt-1">{child.email}</p>
                )}
                {child.grade_level && (
                  <p className="font-body text-primary font-semibold text-sm mt-0.5">{child.grade_level}</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 bg-surface-container-low p-2 rounded-xl">
              <button className="flex flex-col items-center justify-center py-4 px-2 rounded-lg bg-surface-container-lowest shadow-sm hover:shadow-md transition-shadow group active:scale-95">
                ...
                <span className="font-headline font-bold text-xs text-on-surface">الجدول</span>
              </button>
              ...
              >
                ...
                <span className="font-headline font-bold text-xs text-on-surface">المعلمون</span>
              </button>
              ...
              <button className="flex flex-col items-center justify-center py-4 px-2 rounded-lg bg-surface-container-lowest shadow-sm hover:shadow-md transition-shadow group active:scale-95">
                ...
                <span className="font-headline font-bold text-xs text-on-surface">الواجبات</span>
              </button>
              ...
              <button className="flex flex-col items-center justify-center py-4 px-2 rounded-lg bg-surface-container-lowest shadow-sm hover:shadow-md transition-shadow group active:scale-95">
                ...
                <span className="font-headline font-bold text-xs text-on-surface">التقارير</span>
              </button>
            </div>
        ...
        {initialChildren.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center bg-surface-container-lowest rounded-2xl border border-outline-variant/10 shadow-sm">
            ...
            <h3 className="font-headline text-xl font-bold text-on-surface mb-2">لا يوجد أبناء مضافين</h3>
