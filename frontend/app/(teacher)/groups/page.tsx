...
export default async function GroupsPage() {
  ...
  try {
    paginatedGroups = await GroupsApi.fetchGroups();
  } catch (err: unknown) {
  ...
  return (
    <main className="p-4 max-w-xl mx-auto rtl pb-20">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">المجموعات التعليمية</h1>
        <Link 
          href="/groups/new" 
      ...
      {error ? (
        ...
        <div className="space-y-4">
          {paginatedGroups?.items && paginatedGroups.items.length > 0 ? (
            paginatedGroups.items.map((group: Group) => (
            ...
            <div className="text-center text-gray-500 py-12 bg-gray-50 rounded-xl border border-gray-100 border-dashed">
              ...
              <p className="text-xs text-gray-400 mt-1">اضغط على إضافة مجموعة للبدء</p>
            </div>
          )}
        </div>
      )}
