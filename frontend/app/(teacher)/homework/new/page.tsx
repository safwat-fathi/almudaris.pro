...
/**
 * @deprecated - use the AddHomeWorkSheet component instead
 */
export default async function CreateHomeworkPage({ searchParams }: PageProps) {
  const resolvedSearchParams = await searchParams;
  const sessionId = resolvedSearchParams.sessionId as string | undefined;
  const educationStage = resolvedSearchParams.education_stage as
		| string
		| undefined;
	const educationYear = resolvedSearchParams.education_year as
		| string
		| undefined;
  const backUrl = sessionId ? `/sessions/${sessionId}` : "/homework";

	console.warn(
		"CreateHomeworkPage is deprecated. Use AddHomeWorkSheet instead.",
	);
		...
		<div className="bg-surface text-on-surface antialiased min-h-screen flex flex-col pb-28">
			...
			<header className="fixed top-0 w-full z-50 bg-slate-50/85 backdrop-blur-md flex justify-between items-center px-6 h-16 shadow-none">
				<div className="flex items-center gap-3">
					<Link
						href={backUrl}
						className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-slate-200/50 transition-colors text-slate-600 active:scale-95"
					>
			...
			<main className="pt-20 px-6 md:px-20 max-w-2xl mx-auto w-full grow">
				...
				>
					...
					<select
						...
						className="h-12 rounded-xl border border-outline-variant/30 bg-surface-container-lowest px-3"
					>
					...
					<button
						type="submit"
						className="h-12 rounded-xl bg-primary text-on-primary font-bold"
					>
						تصفية
					</button>
				</form>
				...
				<header className="mb-8">
					...
					<p className="text-on-surface-variant font-body text-sm">
						قم بتعبئة تفاصيل الواجب المدرسي للدفعة المختارة
					</p>
				</header>
				...
				<form className="space-y-6">
					...
					<div className="space-y-2">
						...
						<p className="text-xs text-on-surface-variant mr-1">
							سيتم تعيين الواجب لكل طلاب الدفعة
						</p>
					</div>
					...
					<div className="space-y-2">
						...
						<div className="group relative flex flex-col items-center justify-center w-full min-h-[180px] bg-white border-2 border-dashed border-outline-variant hover:border-primary rounded-3xl transition-all p-6 cursor-pointer overflow-hidden">
							...
							<div className="relative z-10 flex flex-col items-center text-center">
								...
								<p className="font-bold text-on-surface mb-1 font-body text-sm">
									اضغط هنا أو قم بسحب الملفات
								</p>
								<p className="text-xs text-on-surface-variant font-body">
									الحد الأقصى للملف 25 ميجابايت
								</p>
							</div>
						</div>
					</div>
					...
					<div className="space-y-2">
						...
						<div className="flex gap-2 mt-3 overflow-x-auto pb-2 scrollbar-none">
							<button
								type="button"
								className="whitespace-nowrap px-4 py-1.5 bg-primary text-on-primary text-xs rounded-full font-bold active:scale-95 transition-transform"
							>
								اليوم
							</button>
							<button
								type="button"
								className="whitespace-nowrap px-4 py-1.5 bg-surface-container-high text-on-surface-variant text-xs rounded-full font-bold active:scale-95 transition-transform hover:bg-surface-container-highest"
							>
								غداً
							</button>
							<button
				...
				<div className="mt-12 p-6 bg-blue-50/50 rounded-3xl flex gap-4 items-start border border-blue-100">
					...
					<div>
						<h4 className="font-extrabold text-primary text-sm mb-1 font-headline">
							نصيحة
						</h4>
						<p className="text-blue-800/80 text-sm leading-relaxed font-body">
							تخصيص تاريخ تسليم محدد يساعد الطلاب على تنظيم وقتهم بشكل أفضل
							وتلقي إشعارات تذكيرية قبل الموعد النهائي.
						</p>
					</div>
				</div>
			</main>
