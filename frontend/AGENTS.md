...
- `pnpm run lint`: runs eslint and prettier on the codebase
- `pnpm run prettier —write path/to/file.tsx` for single file format instead of formatting the whole project
...
- After any changes run `pnpm run lint` to check for lint errors
...
export const metadata: Metadata = {
	...
	description: 'Al-Mudaris Pro Application',
}
...
- Use Server Actions for form submissions and Zod schema for validations along with CSRF token for security as hidden input. Also use server actions on FE for all forms.
