...
async function bootstrap() {
  ...
  const options = new DocumentBuilder()
    .setTitle('Al-Mudaris Pro API')
    .setDescription('Al-Mudaris Pro API documentation')
    .setVersion('0.0.1')
    .setExternalDoc('API Documentation', '/docs')
    .setContact(
      'Al-Mudaris Pro',
      'https://www.almudaris.pro',
      'help@almudaris.pro',
    )
    .addBearerAuth(
      {
  ...
  SwaggerModule.setup('docs', app, document, {
    ...
    customSiteTitle: 'Al-Mudaris Pro API docs - ' + process.env.NODE_ENV,
  ...
  const userRlsInterceptor = app.get(UserRlsInterceptor);
